...
class DB_Functions {
    ...
    public function create_post($kriteria_postingan, $title, $description, $created_by, $daerah_asal, $daerah_tujuan, $jumlah_orang) {

        $stmt = $this->conn->prepare("INSERT INTO ngetrip_post(kriteria_postingan, title, description, created_by, created_at, daerah_asal, daerah_tujuan, jumlah_orang) VALUES(?, ?, ?, ?, NOW(), ?, ?, ?)");
        $stmt->bind_param("ississi", $kriteria_postingan, $title, $description, $created_by, $daerah_asal, $daerah_tujuan, $jumlah_orang);

        $result = $stmt->execute();
        $stmt->close();

        if ($result) {
            $stmt = $this->conn->prepare("SELECT * FROM ngetrip_post WHERE description = ?");
